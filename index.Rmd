...
$$ 2. Finally, calculate the final enrichment score ($ES$): $$ES = max_{i}(es_{i}), \quad \quad \quad (1.1)$$ the maximum positional enrichment score. Here, $N_{R}$ and $N_{S_{X}}$ are the number of genes in the drug ($R$) and disease ($S_{X}$) gene sets. $w_{ES}$ is the weight assigned to each position in the drug profile $\vec{R}$. When $w_{ES}=0$, $N_{S_{X}^{'}} = \displaystyle\sum_{\substack{gr_{j} \in S}}|v_{drg}(gr_{j})|^{0} = N_{S},$ which results in $$
...
Samart K$^{*}$, Tuyishime P$^{*}$, Krishnan A^, Ravi J^. Reconciling multiple connectivity <br>
        scores for drug repurposing. *arXiv*(2020) [2009.09317](arxiv.org/abs/2009.09317) <br>
