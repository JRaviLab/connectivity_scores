...
|                                      |                                                                                                                                                                                                                                              |
| $S^{+}_{X}$, $S^{-}_{X}$             | the up- and down-regulated subsets of SX, the genes with the extreme disease gene scores; $S_{X}^{+} \cup S_{X}^{-} = S_{X}$                                                                                                                 |
...
P_{hit}(S_{X},i) = \displaystyle\sum_{\substack{gr_{j} \in S_{X} \\ j \leq i}}\frac{|v_{drg}(gr_{j})|^{w_{ES}}}{N_{S_{X}}^{'}},\qquad where \qquad N_{S_{X}^{'}} = \displaystyle\sum_{gr_{j} \in S_{X}}|v_{drg}(gr_{j})|^{w_{ES}} 
...
$$ 2. Finally, calculate the final enrichment score ($ES$): $$ES = max_{i}(es_{i}),$$ the maximum positional enrichment score. Here, $N_{R}$ and $N_{S_{X}}$ are the number of genes in the drug ($R$) and disease ($S_{X}$) gene sets. $w_{ES}$ is the weight assigned to each position in the drug profile $\vec{R}$. When $w_{ES}=0$, $N_{S_{X}^{'}} = \displaystyle\sum_{\substack{gr_{j} \in S}}|v_{drg}(gr_{j})|^{0} = N_{S},$ which results in $$
...
ES = max(P_{hit}(S_{X},i)-P_{miss}(S_{X},i)) = sign(P_{hit}(S_{X},i)-P_{miss}(S_{X},i)) \times KS 
$$ where $$
...
| $N_{S_{X}^{'}}$        | $N_{R}$             | the sum of absolute drug gene score ($v_{drg}(gr_{j})$) of every $\vec{R}$ gene in $S_{X}$ weighted by $w_{ES}$         |
...
$$
css(\vec{R},\vec{S}) = \sum^{N_{S}}_{\substack{i=1}}[(r^{abs}_{drg}(gs_{i}) \times sgn_{drg}(gs_{i})) \times (r^{abs}_{dis}(gs_{i}) \times sgn_{dis}(gs_{i}))]
$$
...
CSS = \frac{css(\vec{R},\vec{S})}{css^{o}_{max}(N_{R},N_{S})}
$$
...
css(\vec{R},S) = \sum^{N_{S}}_{\substack{i=1}}[r^{abs}_{drg}(gs_{i}) \times sgn_{drg}(gs_{i})]
$$

$$
CSS = \frac{css(\vec{R},S)}{css^{u}_{max}(N_{R},N_{S})}
